...
function openEnvelope(envelope: Envelope) {
  router.push({ name: 'envelope', params: { id: envelope.id } })
}
</script>
  ...
  <div class="flex flex-col gap-7">
    ...
    <template v-else-if="envelopesStore.envelopes.length > 0">
      ...
      <div class="grid grid-cols-[repeat(auto-fill,minmax(250px,1fr))] gap-2.5">
        <div
          v-for="envelope in sortedEnvelopes"
          :key="envelope.id"
          role="button"
          tabindex="0"
          class="neu-surface flex cursor-pointer flex-col gap-4 bg-default p-5"
          @click="openEnvelope(envelope)"
          @keydown.enter="openEnvelope(envelope)"
        >
