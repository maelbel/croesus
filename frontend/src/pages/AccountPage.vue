...
const account = computed<Account | null>(
  () => accountsStore.accounts.find((a) => a.id === Number(props.id)) ?? null,
)

// Covers both a bad id in the URL and an account deleted (from another tab,
// or via this page's own delete button) while its page is still open.
watch(
  () => [account.value, accountsStore.loading] as const,
  ([found, loading]) => {
    if (!found && !loading) router.replace('/accounts')
  },
  { immediate: true },
)

usePageTitle(
  () => (account.value ? accountTypeLabel(account.value.type) : ''),
  () => account.value?.name ?? '',
)
...
const accountValuations = computed(() => {
  ...
  return [...(valuationsStore.byAccount.get(account.value.id) ?? [])].reverse()
})

const accountAssets = computed(() => (account.value ? assetsStore.forAccount(account.value.id) : []))
const hasHoldings = computed(() => accountAssets.value.length > 0)

const currentValue = computed(() => (account.value ? valuationsStore.currentValue(account.value.id) : 0))
const change30d = computed(() => (account.value ? valuationsStore.changeOverDays(account.value.id, 30) : null))

const emergencyRatio = computed(() => {
  const target = Number(account.value?.emergency_fund_target ?? 0)
...
// account this route currently points at.
const accountForm = useCrudForm<Account, AccountCreate>({
  ...
  toFormValues: (a) => ({
    ...
    emergency_fund_target: a.emergency_fund_target,
    notes: a.notes,
  }),
...
watch(
  account,
  (a) => {
    if (a) accountForm.openEdit(a)
  },
  { immediate: true },
)

// Starts collapsed to a read-only summary — most visits are to check the
// balance/history, not to change the account's own fields. "Edit" reveals
// the same fields in place (still no modal), "Save"/"Cancel" collapse it
// back so the page doesn't stay stuck in form mode after a successful save.
const editingDetails = ref(false)

async function saveAccountDetails() {
  if (await accountForm.submit()) editingDetails.value = false
}

function cancelEditDetails() {
  if (account.value) accountForm.openEdit(account.value)
...
async function removeAccount() {
  if (!account.value) return
  await deleteAccount(t('accounts.deleteConfirm', { name: account.value.name }), async () => {
...
const valuationForm = useCrudForm<Valuation, ValuationFormValues, ValuationUpdate>({
  ...
  create: (payload) => valuationsStore.create({ ...payload, account_id: account.value!.id }),
...
const assetForm = useCrudForm<Asset, AssetFormValues, AssetUpdate>({
  ...
  create: (payload) => assetsStore.create({ ...payload, account_id: account.value!.id }),
  update: (id, payload) => assetsStore.update(id, payload),
})

const activeTab = ref<'history' | 'holdings'>('history')

// A fresh id in the route (navigating from one account's page straight to
// another's) should reset both forms and land back on the History tab.
watch(
  () => props.id,
  () => {
...
<template>
  <div v-if="account" class="flex flex-col gap-8">
    <RouterLink to="/accounts" class="inline-flex w-fit items-center gap-1 text-sm text-muted hover:text-toned">
    ...
    <section class="neu-surface flex flex-col gap-4 p-5">
      <div class="flex flex-col gap-1">
        <span class="text-xs tracking-wide text-muted uppercase">{{ t('accountDetail.currentValue') }}</span>
        <span class="font-heading text-[40px] leading-none font-extrabold">{{ formatCurrency(currentValue, account.currency) }}</span>
    ...
    <section class="flex flex-col gap-4">
      <div class="flex items-center justify-between gap-4">
        <span class="text-xs tracking-wide text-muted uppercase">{{ t('accountDetail.detailsHeading') }}</span>
        <UButton
          v-if="!editingDetails"
          variant="ghost"
          color="neutral"
      ...
      <div class="neu-surface flex flex-col gap-4 p-5">
        <template v-if="!editingDetails">
          <dl class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div class="flex flex-col gap-0.5">
              <dt class="text-xs text-muted">{{ t('accounts.fieldType') }}</dt>
              <dd class="text-[15px]">{{ accountTypeLabel(account.type) }}</dd>
            </div>
            <div class="flex flex-col gap-0.5">
              <dt class="text-xs text-muted">{{ t('accounts.fieldCurrency') }}</dt>
              <dd class="text-[15px]">{{ account.currency }}</dd>
            </div>
            <div v-if="account.institution" class="flex flex-col gap-0.5">
              <dt class="text-xs text-muted">{{ t('accounts.fieldInstitution') }}</dt>
              <dd class="text-[15px]">{{ account.institution }}</dd>
            </div>
            <div v-if="account.opened_at" class="flex flex-col gap-0.5">
              <dt class="text-xs text-muted">{{ t('accounts.fieldOpenedOn') }}</dt>
              <dd class="text-[15px]">{{ formatDate(account.opened_at) }}</dd>
            </div>
          </dl>
        ...
        <template v-else>
          ...
          <div class="flex flex-col gap-3">
            ...
            <UFormField v-if="accountForm.state.form.is_emergency_fund" :label="t('accounts.fieldEmergencyTarget')" class="max-w-xs">
          ...
          <div class="flex items-center gap-2.5">
            <UButton :loading="accountForm.state.submitting" @click="saveAccountDetails">
              {{ t('common.save') }}
            </UButton>
            <UButton variant="ghost" color="neutral" :disabled="accountForm.state.submitting" @click="cancelEditDetails">
              {{ t('common.cancel') }}
            </UButton>
          </div>
        </template>
        ...
        <div class="flex flex-wrap items-center justify-between gap-3 border-t border-default pt-4">
          <span class="text-sm text-muted">{{ t('accountDetail.deleteAccountHint') }}</span>
          <UButton color="rust" variant="outline" size="sm" class="flex-none" @click="removeAccount">
